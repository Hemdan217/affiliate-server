...
const openUser = async (req, res) => {
  try {
    ...
    if (!user.is_active) return res.status(400).json({ success: false, errors: [{ msg: 'لم يتم تفعيل حسابك بعد!' }] });

    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1d' });
    if (!token) return res.status(400).json({ success: false, errors: [{ msg: 'حدث خطا ما' }] });
...
const updateUserPassword = async (req, res) => {
  try {
    ...
    if (req.body.new_password !== req.body.confirm_new_password) {
      return res.status(400).json({ success: false, errors: [{ 'msg': 'password must be the same!' }] });
    }
    ...
    if (!verifyPassword) {
      return res.status(400).json({ success: false, errors: [{ 'msg': 'try again later' }] });
    }

    const hashPassword = await bcrypt.hash(req.body.new_password, 10);

    const newRecord = await User.findByIdAndUpdate(user?._id, {
      password: hashPassword
    }, { new: true });
